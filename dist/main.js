...
var PrChecker = class {
  constructor(prNumber, labels, requiredLabels, ghToken, jiraRegex) {
    ...
    this.octokit = import_github.default.getOctokit(ghToken);
  }
  async run() {
    this.checkLabels();
    this.checkTitle();
  ...
  async checkTitle() {
    if (!this.jiraRegExp.test(import_github.default.context.payload.pull_request.title)) {
  ...
  createComment(body) {
    this.octokit.rest.issues.createComment(__spreadProps(__spreadValues({}, import_github.default.context.repo), {
      issue_number: this.prNumber,
      body
    }));
  }
  async isLastComment(message) {
    const comments = await this.octokit.rest.issues.listComments(__spreadProps(__spreadValues({}, import_github.default.context.repo), {
      issue_number: this.prNumber
    }));
